...
import { WPDataTable, PageTitle, WPModal2 } from "../../../components";
import { Divider, Tag, Form, Button } from "antd";

class EventCustomers extends Component {
  constructor(props) {
    super(props);
    this.state = {
      data: [],
  ...
  setToggles = e => {
    ...
    const promise = fetch(url, {
      ...
      body: JSON.stringify({
        ...
        userName: this.state.data[this.state.idx].userName,
        address1: this.state.data[this.state.idx].address1,
  ...
  render() {
    let newData = this.state.data.filter(function (element) {
      if (element.eventClassification === "행사" && element.contractClassification !== "취소" && element.contractClassification !== "삭제") {
        element.groomName = "행사이름";
        return element
      }
    ...
    return (
      <div>
        <Event  {...this.state} onChange={this.onChange} values={this.state.values} details={this.state.details}
        ...
        <tfoot>
          <tr>
            <td colSpan="2" style={{ padding: 10, textAlign: "right" }}>
              ...
              <Button onClick={() => console.log(props.idKey)} type="primary">검색</Button>
            </td>
          </tr>
        </tfoot>
      </table>
