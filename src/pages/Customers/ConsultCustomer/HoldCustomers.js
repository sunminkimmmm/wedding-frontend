...
class HoldCustomers extends Component {
  constructor(props) {
    super(props);
    this.state = {
      data: [],
  ...
  render() {
    let newData = this.state.data.filter(function (element) {
      ...
      return element.state === "보류고객";
    });


    return (
      <div>
        <Hold {...this.state} onChange={this.onChange} newData={newData}
