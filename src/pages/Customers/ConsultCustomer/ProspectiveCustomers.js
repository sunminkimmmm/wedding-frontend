...
class ProspectiveCustomers extends Component {
  constructor(props) {
    super(props);
    this.state = {
      data: [],
  ...
  render() {
    let newData = this.state.data.filter(function (element) {
      ...
      return element.state === "유망고객";
    });


    return (
      <div>
        <Prospective {...this.state} onChange={this.onChange} newData={newData}
